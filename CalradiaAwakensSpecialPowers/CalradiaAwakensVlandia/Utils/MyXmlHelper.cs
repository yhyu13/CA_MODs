...
namespace CalradiaAwakensSpecialPowers.Utils
{
	public static class MyXmlHelper
	{
		public static T SettingsFor<T>(string fileName)
		{
			...
			try
			{
				using (XmlReader xmlReader = XmlReader.Create(inputUri))
				{
					...
					xmlRootAttribute.ElementName = Main.ModNamePath;
		...
		public static void ReadEquipmentSpecialPowerFile(string fileName, ref Dictionary<string, HashSet<string>> dict)
		{
			string inputUri = Path.Combine(BasePath.Name, "Modules", Main.ModNamePath, "settings", $"{fileName}.xml");
			try
			{
				...
				foreach (var pair in elements)
				{
					var key = pair.Attribute("id").Value;
					var value = pair.Attribute("text").Value;
